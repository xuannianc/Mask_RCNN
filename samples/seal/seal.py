...
class SealDataset(utils.Dataset):
    ...
    def load_seal(self, dataset_dir, subset):
        ...
        # 三个参数分别是: source, class_id, class_name
...
def remove(model, image_path=None, video_path=None):
    assert image_path or video_path
    ...
    if image_path:
        ...
        if masks.shape[-1] > 0:
            ...
            ones = np.ones((image.shape)) * 255
            clipped_image = np.where(masks, ones, image).astype(np.uint8)
        else:
            clipped_image = image
        cv2.namedWindow('clipped_image', cv2.WINDOW_NORMAL)
        cv2.imshow('clipped_image', clipped_image)
        cv2.waitKey(0)
    elif video_path:
        # Video capture
        vcapture = cv2.VideoCapture(video_path)
